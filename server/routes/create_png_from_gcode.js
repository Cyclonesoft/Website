...
function process_file(
    ...
    fs.readFile(__dirname + "/../../files/" + title + ".gcode", 'utf8', function (err, code) {
        ...
        sharp(Buffer.from(svg)).resize(500).toFile(__dirname + "/../../files/" + title + ".png", (err, info) => {
            if (err) {
                console.log("No se puede escribir" + title + ".png .");  //"Cannot write " + title + ".png file."
            } else {
                console.log('Conversion finalizada ' + title + '.gcode a png'); //Done converting ' + title + '.gcode to png'
                callback();
            }
        });
    });
