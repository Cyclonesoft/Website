...
const port = new SerialPort(PORT, {
    baudRate: 115200 //250000 era antes este lo cambie para probar
...
function sendNextCmd(justExecuteWithoutReadingPort = false, dat) {
    ...
    if (data.toString('utf8').includes("ok")) {
        if (cmdQueue.length > 0) {
            ...
            port.write(cmd + "\n");
            if (DEBUG) console.log("Enviando CMD: " + cmd);
            ...
            if (DEBUG) console.log("Enviando CMD: " + file[index]);
            index += backwards ? -1 : 1;
        } else {
            if (file.length > 0)
                doneCallback();
...
function sendFile(filename) {
    ...
    setTimeout(() => {
        ...
        resume();
        sendNextCmd(true);
        console.log("Iniciando Patron: " + filename);
...
function setSpeed(newSpeed) {
    ...
    console.log("Velocidad Fijada a:" + newSpeed + "%"); //"Set speed to "
}
...
setTimeout(() => {
    ...
    console.log("Controlador GCode Inicializado."); //GCode Controller Initialized
}, 5000);
