...
router.post('/', function (req, res) {
  if (!req.files || Object.keys(req.files).length === 0) {
    return res.status(400).send('Ningun archivo fue cargado'); //'No files were uploaded.'
  }
  ...
  if (!thrFile.name.endsWith(".thr")) {
    return res.status(400).send("Tipo de archivo invalido."); //"Invalid file type"
  }
            ...
            (err, info) => {
              sharp(__dirname + "/../../files/" + filename + ".png")
                ...
                .toFile(__dirname + "/../../files/" + filename + "-small.png",
                  (err, info) => {
                    res.send('Archivo Actualizado!!!'); // File uploaded!
                  }
                );
            });
      });
